...
type cdnRedirectFlagTest struct {
	name     string
	redirect bool
}

func TestAccessController_CDNRedirectFlag(t *testing.T) {
	req, err := http.NewRequest("GET", "https://registry.gitlab.com/v2/myrepo/blobs/sha256:7b1a6ab2e44dbac178598dabe7cff59bd67233dba0b27e4fbd1f9d4b3c877a54", nil)
	require.NoError(t, err)
	ctx := dcontext.WithRequest(dcontext.Background(), req)
	...
	tt := []cdnRedirectFlagTest{
		{name: "no flag", redirect: false},
		{name: "false flag", redirect: false},
		{name: "true flag", redirect: true},
	}
	...
	for _, test := range tt {
		t.Run(test.name, func(t *testing.T) {
			actions := []*ResourceActions{{
				...
				Name:        access.Name,
				Actions:     []string{access.Action},
				CDNRedirect: test.redirect,
			}}

			authCtx := newTestMigrationAuthContext(t, ctx, req, actions, access)
			require.Equal(t, test.redirect, dcontext.ShouldRedirectToCDN(authCtx))
...
func TestAccessController_CDNRedirectFlag_MultipleAccess(t *testing.T) {
	url := "https://registry.gitlab.com/v2/myrepo-b/blobs/uploads/?mount=sha256:f72c9c3fbe478eac9b924b6bec463b6b1e0a5ebcd902085662a29f8a20df327d&from=myrepo-a"
	req, err := http.NewRequest("POST", url, nil)
	require.NoError(t, err)
	ctx := dcontext.WithRequest(dcontext.Background(), req)
